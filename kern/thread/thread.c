...
thread_switch(threadstate_t newstate, struct wchan *wc, struct spinlock *lk)
{
	 ...
	 *															//So when a thread is being added to the runqueue of the current cpu, since we hold the lock to the queue, another process cannot end on the queue that was already running on the same cpu? Maybe it finishes the job but the "State" of the thread isn't updated as it is not removed from the queue
