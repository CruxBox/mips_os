...
void male_start(uint32_t index)
{
	(void)index;
	lock_acquire(testlock);
...
void female_start(uint32_t index)
{
	(void)index;
	lock_acquire(testlock);
	check_thread(whale_threads, index);
...
void female_end(uint32_t index)
{
	(void)index;
	lock_acquire(testlock);
	check_thread(whale_threads, index);
...
void matchmaker_start(uint32_t index)
{
	(void)index;
	P(matcher_sem);
	lock_acquire(testlock);
...
void matchmaker_end(uint32_t index)
{
	(void)index;
	lock_acquire(testlock);
	check_thread(whale_threads, index);
...
void inQuadrant(int quadrant, uint32_t index)
{
	random_yielder(PROBLEMS_MAX_YIELDER);
	random_spinner(PROBLEMS_MAX_SPINNER);
	lock_acquire(testlock);
...
void leaveIntersection(uint32_t index)
{
	random_yielder(PROBLEMS_MAX_YIELDER);
	random_spinner(PROBLEMS_MAX_SPINNER);
	lock_acquire(testlock);
